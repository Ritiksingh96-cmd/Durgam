      ...
      if (mainContent) {
        mainContent.innerHTML = `
          <div class="gov-card" style="max-width:720px; margin:40px auto; text-align:center; padding:40px; border-top:5px solid var(--gov-danger); box-shadow:0 10px 30px rgba(0,0,0,0.08);">
            <div style="width:64px; height:64px; background:#FEE2E2; color:#C62828; border-radius:50%; display:inline-flex; align-items:center; justify-content:center; margin-bottom:16px;">
              <svg class="gov-icon gov-icon-xl" viewBox="0 0 24 24" style="stroke:#C62828;"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
            </div>
      ...
      if (authBadgeSlot) {
        authBadgeSlot.innerHTML = `
          <div style="background:#ECFDF5; border:1px solid #A7F3D0; padding:6px 14px; border-radius:10px; font-size:11.5px; font-weight:800; color:#065F46; display:flex; align-items:center; gap:10px;">
            <svg class="gov-icon gov-icon-sm" viewBox="0 0 24 24" style="stroke:#1B5E20;"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            <div>
              <div>${session.full_name} (${session.badge_number})</div>
              <div style="font-size:9.5px; color:#1B5E20; font-weight:600;">${session.jurisdiction}</div>
            </div>
            <button onclick="logoutUser()" class="btn-utility" style="background:#C62828; color:#FFFFFF; border:none; padding:3px 8px; font-size:10px; margin-left:6px;">Log Out</button>
          </div>
        `;
      }
    }
...
function initMultiBranchMoneyTrailCanvas() {
  ...
  function renderGraph() {
    ...
    const nodes = cur?.nodes || [
      { id: "0", label: "Source: Remitter", bank: "SBI (Delhi NCR)", account: "XXXX-2948", type: "Savings", amount: "₹2,50,000", risk: "Source Remitter", color: "#1565C0", x: 0.08, y: 0.5 },
      { id: "1A", label: "Hop 1A: Layer 1 Mule", bank: "PNB (Mewat, HR)", account: "XXXX-9541", type: "Current", amount: "₹1,50,000", risk: "92% Mule Risk", color: "#F9A825", x: 0.38, y: 0.28 },
      { id: "1B", label: "Hop 1B: Layer 1 Mule", bank: "Canara (Gurugram)", account: "XXXX-3184", type: "Savings", amount: "₹1,00,000", risk: "88% Mule Risk", color: "#F9A825", x: 0.38, y: 0.72 },
      { id: "2", label: "Hop 2: Aggregator Mule", bank: "ICICI (Chandigarh)", account: "XXXX-8931", type: "Current", amount: "₹2,50,000", risk: "94% Mule Risk", color: "#1565C0", x: 0.68, y: 0.5 },
      { id: "3", label: "Terminal Cashout ATM", bank: "SBI ATM (Connaught Place)", account: "XXXX-4821", type: "ATM Kiosk", amount: "₹2,50,000", risk: "✓ MICRO-HOLD (138ms)", color: "#C62828", x: 0.92, y: 0.5 }
    ];
    ...
    nodes.forEach(n => {
      ...
      // Amount & Risk
      ctx.fillStyle = n.id.includes('3') || n.id === '2' && nodes.length === 3 ? (DURGAM_STATE.isHoldDissolved ? '#C62828' : '#1B5E20') : '#1565C0';
...
function initGeospatialATMHeatmap() {
  ...
  const atmIcon = L.divIcon({
    ...
    html: `
      <div style="background:#C62828; color:white; border:2px solid #FFF; border-radius:50%; width:38px; height:38px; display:flex; align-items:center; justify-content:center; font-weight:bold; box-shadow:none; font-size:18px;">
        🏧
      </div>
    `,
    iconSize: [38, 38],
  ...
  DURGAM_STATE.atmMarker.bindPopup(`
    <div style="font-size:12.5px; font-family:Inter,sans-serif; padding:4px;">
      <strong style="color:#0F4C81;">ATM Cashout Terminal Predicted</strong><br>
      <span style="color:#C62828; font-weight:bold;">Probability: 94.6% (CRITICAL HOTSPOT)</span><br>
      <span style="color:#455A64;">CCTV Stream: Active • Cash Level: ₹4,80,000</span>
    </div>
  `).openPopup();
  ...
  const pcrIcon = L.divIcon({
    ...
    html: `
      <div style="background:#0F4C81; color:#F9A825; border:2px solid #F9A825; border-radius:8px; width:40px; height:40px; display:flex; align-items:center; justify-content:center; font-weight:bold; box-shadow:none; font-size:18px;">
        🚔
      </div>
    `,
    iconSize: [40, 40],
...
function updateHeatmapDensity(centerLat, centerLon) {
  ...
  const heatPoints = [
    { lat: centerLat, lon: centerLon, rad: 800, color: '#D32F2F', op: 0.35 },
    { lat: centerLat + 0.004, lon: centerLon - 0.003, rad: 550, color: '#7CB342', op: 0.25 },
    { lat: centerLat - 0.005, lon: centerLon + 0.004, rad: 450, color: '#F9A825', op: 0.2 },
    { lat: centerLat + 0.008, lon: centerLon + 0.002, rad: 350, color: '#7CB342', op: 0.15 }
  ];
  ...
  function triggerToast() {
    ...
    toast.innerHTML = `
      <div style="flex:1;">
        ...
        <div style="font-size:11px; color:#455A64; margin-top:2px; line-height:1.4;">${n.desc}</div>
      </div>
      <button onclick="this.parentElement.remove()" style="background:none; border:none; color:#90A4AE; cursor:pointer; font-size:14px; padding:0 4px;">✕</button>
    `;
    ...
    DURGAM_STATE.charts.telPie = new Chart(telPieCtx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['#C62828', '#FF6200', '#F9A825', '#00ACC1', '#1565C0', '#455A64'],
          borderWidth: 2,
          borderColor: '#FFFFFF'
        }]
      },
      options: {
        ...
        plugins: {
          legend: { position: 'right', labels: { boxWidth: 12, color: '#263238', font: { size: 10, weight: 'bold' } } }
        }
      }
    });
  }
  ...
  if (telActionCtx) {
    DURGAM_STATE.charts.telAction = new Chart(telActionCtx, {
      ...
      data: {
        ...
        datasets: [{
          data: [94.2, 4.1, 1.7],
          backgroundColor: ['#1B5E20', '#F9A825', '#C62828'],
          borderWidth: 2,
          borderColor: '#FFFFFF'
        }]
      },
  ...
  if (pieCtx) {
    DURGAM_STATE.charts.pie = new Chart(pieCtx, {
      ...
      options: {
        ...
        maintainAspectRatio: false,
        plugins: {
          legend: { position: 'bottom', labels: { color: '#263238', font: { size: 10.5, weight: 'bold' } } }
        }
      }
    });
  }
  ...
  if (lineCtx) {
    DURGAM_STATE.charts.line = new Chart(lineCtx, {
      ...
      options: {
        ...
        maintainAspectRatio: false,
        plugins: {
          legend: { position: 'top', labels: { color: '#263238', font: { size: 10.5, weight: 'bold' } } }
        }
      }
    });
  }
  ...
  function updateStepHeader(step) {
    document.querySelectorAll('.wizard-step-pill').forEach((pill, idx) => {
      if (idx + 1 === step) {
        ...
        pill.style.background = '#0F4C81';
        pill.style.color = '#FFFFFF';
      } else {
        pill.classList.remove('active');
        pill.style.background = '#F8FAFC';
        pill.style.color = '#455A64';
      }
    });
  ...
  if (btnTracker && btnFile && btnAadhaar) {
    [
      ...
      if (item.tab === tabName) {
        ...
        item.btn.style.background = '#0F4C81';
        item.btn.style.color = '#FFFFFF';
        item.btn.style.border = 'none';
      } else {
        item.btn.classList.remove('active');
        item.btn.style.background = '#F8FAFC';
        item.btn.style.color = '#0F4C81';
        item.btn.style.border = '1px solid #D9E2EC';
      }
    });
  }
    ...
    <div class="officer-modal-box">
      <div class="officer-modal-header">
        <div style="display:flex; align-items:center; gap:10px;">
          ...
          <div>
            <div style="font-size:10px; font-weight:800; text-transform:uppercase; color:#F9A825;">GOVERNMENT OF INDIA • JANPARICHAY RBAC</div>
      ...
      <div class="officer-modal-body">
        ...
        <div style="display:flex; gap:10px; margin-top:20px; flex-wrap:wrap;">
          ...
          <button onclick="regenerate2FAToken()" class="btn-utility" style="background:#1B5E20; color:#FFFFFF; border:none; padding:8px 14px;">
...
function selectSSORole(role) {
  document.querySelectorAll('.sso-role-tab').forEach(t => {
    if (t.getAttribute('data-sso-role') === role) {
      ...
      t.style.background = '#0F4C81';
      t.style.color = '#FFFFFF';
    } else {
      t.classList.remove('active');
      t.style.background = '#FFFFFF';
      t.style.color = '#263238';
    }
  });
